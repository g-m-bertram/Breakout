...
void Game::Init()
{
	...
	ResourceManager::LoadShader("resources/shaders/sprite.vs", 
								...
								NULL, "sprite");
	ResourceManager::LoadShader("resources/shaders/particle.vs",
								"resources/shaders/particle.fs",
								NULL, "particle");

	// config shaders
	// ortho creates a matrix for projecting 2D coords on screen
	glm::mat4 projection = glm::ortho(	0.0f, (float)this->Width,	// left, right
										(float)this->Height, 0.0f,	// bottom, top
										-1.0f, 1.0f);				// near, far
...
void Game::Update(float dt)
{
	...
	if (Ball->Position.y >= this->Height)	// failstate
	{
		this->ResetLevel();
		this->ResetPlayer();
	}
...
void Game::Render()
{
	if (this->State == GAME_ACTIVE)
	{
		...
		// draw particles
		//		draw them before ball but after other objects so they appear 
		//		behind ball and in front of other objects
		Particles->Draw();
...
void Game::ResetPlayer()
{
	...
	Player->Position = glm::vec2(this->Width / 2 - PLAYER_SIZE.x / 2.0f, this->Height - PLAYER_SIZE.y);
	Ball->Reset(Player->Position + glm::vec2(PLAYER_SIZE.x / 2.0f - BALL_RADIUS, -(BALL_RADIUS * 2.0f)), INITIAL_BALL_VELOCITY);
}


// forward declarations of functions for DoCollisions()
...
void Game::DoCollisions()
{
	...
	// check collision for player and ball
	Collision result = CheckCollision(*Ball, *Player);
	if (!Ball->Stuck && std::get<0>(result))
	{
		// x component of velocity updates based on how far from center of paddle it hits
...
// calculates direction a vector is facing
Direction VectorDirection(glm::vec2 target)
{
	glm::vec2 compass[] =
	{
		glm::vec2( 0.0f,  1.0f),	// N
		glm::vec2( 1.0f,  0.0f),	// E
		glm::vec2( 0.0f, -1.0f),	// S
		glm::vec2(-1.0f,  0.0f)		// W
	};
	float max = 0.0f;
